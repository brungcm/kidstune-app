...
Pássaros a voar, flor a desabrochar...`;
        ...
        await (0, generate_1.generateHandler)(req, res);
...
describe("POST /api/free-quota", () => {
    ...
    test("returns 429 when quota exhausted", async () => {
        ...
        const res = createMockRes();
        await (0, free_quota_1.freeQuotaHandler)(req, res);
        expect(res.status).toHaveBeenCalledWith(429);
        expect(res.json).toHaveBeenCalledWith(expect.objectContaining({ error: "free_quota_exhausted" }));
    });
    test("returns 200 with remaining count for new IP", async () => {
        ...
        await (0, free_quota_1.freeQuotaHandler)(req, res);
        expect(res.status).toHaveBeenCalledWith(200);
        expect(res.json).toHaveBeenCalledWith(expect.objectContaining({ remaining: 1 }));
    });
});
